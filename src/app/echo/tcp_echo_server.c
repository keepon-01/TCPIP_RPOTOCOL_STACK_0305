...
static void tcp_echo_server_thread (void * arg) {
    ...
    while (1) {
        // 等待客户端的连接
        struct sockaddr_in client_addr;//注意这里是客户端的地址
        socklen_t addr_len = sizeof(client_addr);
        int client = accept(s,  (struct sockaddr*)&client_addr, &addr_len);//注意这是传输信息的套接字，不是监听的套接字，套接字这个数字映射到某个结构体，里面会包含ip和端口这个属性
        if (client < 0) {
            printf("accept error");
            break;
        }
